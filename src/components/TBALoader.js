...
export default function TBALoader() {
  ...
  const [teams, loadTeams] = React.useState(
    localStorage.getItem("matches") === null ||
      localStorage.getItem("event_code") !== Config.tba_eventid
      ? ""
      : "Loaded ✅"
  );
  const [matches, loadMatches] = React.useState(
    localStorage.getItem("matches") === null ||
      localStorage.getItem("event_code") !== Config.tba_eventid
      ? ""
      : "Loaded ✅"
      ...
      await axios
        ...
        .then((response) => {
          ...
          for (let i = 0; i < filtered.length; i++) {
            for (
              let j = 0;
              j < filtered[i].alliances.red.team_keys.length;
              j++
            ) {
              var result = JSON.parse(localStorage.getItem("teams")).filter(
                (x) => x.key === filtered[i].alliances.red.team_keys[j]
              );
            ...
            for (
              let j = 0;
              j < filtered[i].alliances.blue.team_keys.length;
              j++
            ) {
              var res = JSON.parse(localStorage.getItem("teams")).filter(
                (x) => x.key === filtered[i].alliances.blue.team_keys[j]
              );
